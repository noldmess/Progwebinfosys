<div class="span9">
	<div class="hero-unit">
		...
		<p><?php if(isset($TEMPLATE['image']) && $TEMPLATE['image'] !== 'NOIMG'){ echo '<img src="'.ltrim(dirname($_SERVER['SCRIPT_NAME']), ' \\').'/art_images/'.$TEMPLATE['id'].'.png" style="float:'.$TEMPLATE['align'].'">';}echo eval('?>'.$TEMPLATE['parsedText'].'<?php ');?></p>
		...
		<a class="btn btn-primary" href="<?php echo ltrim(dirname($_SERVER['SCRIPT_NAME']), ' \\');?>/wiki/<?php echo $TEMPLATE['id']."/".preg_replace( '/\s+/', '_', $item['title']);?>/change"><i class="icon-pencil"></i>change</a><br>
		<?php }
			 $first = true; 
