<div class="span9">
	<div class="hero-unit">
		...
		<a class="btn btn-danger" href="<?php echo str_replace("\\", "/", dirname($_SERVER['SCRIPT_NAME']));?>wiki/<?php echo $TEMPLATE['id']."/".preg_replace( '/\s+/', '_', $item['title']);?>/remove"><i class="icon-remove"></i>remove</a>
		<a class="btn btn-primary" href="<?php echo str_replace("\\", "/", dirname($_SERVER['SCRIPT_NAME']));?>wiki/<?php echo $TEMPLATE['id']."/".preg_replace( '/\s+/', '_', $item['title']);?>/change"><i class="icon-pencil"></i>change</a><br>
		<?php }
			 $first = true; 
