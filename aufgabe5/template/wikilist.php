<div class="span3">
	...
	<ul class="nav nav-list">
		...
		<?php  foreach($TEMPLATE['index'] as $item){?>
				<li><a href="<?php echo str_replace("\\", "/", dirname($_SERVER['SCRIPT_NAME']));?>/wiki/<?php echo $item['id']."/".preg_replace( '/\s+/', '_', $item['title']);?>/"> <?php echo urldecode($item['title']);?></a></li>
		<?php }?>
	...
	<?php if(isset($TEMPLATE['user_id']) && $TEMPLATE['user_id'] !== false){?>
		...
		<li><a href="<?php echo str_replace("\\", "/", dirname($_SERVER['SCRIPT_NAME']));?>/logout/"><button class="btn btn-successs">Logout</button></a></li>
	<?php }else{?>
			<li><a href="<?php echo str_replace("\\", "/", dirname($_SERVER['SCRIPT_NAME']));?>/login/"><button class="btn btn-successs">Login</button></a></li>
	<?php }?>
	</ul>
	<?php 
