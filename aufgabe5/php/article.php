...
class Article{
	...
	public function parse_links($matches){
		...
		if(isset($art['id'])){
			...
			return $matches[1].'<a href="/aufgabe5/wiki/'.$art['id'].'/'.urlencode($matches[2]).'/">'.$matches[2].'</a>'.$matches[3]; 
		}else{
			return $matches[1].'<a href="/aufgabe5/wiki/'.urlencode($matches[2]).'/">'.$matches[2].'</a>'.$matches[3]; 
		}
	}
